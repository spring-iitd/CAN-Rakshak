#!/usr/bin/env python3
"""
MIRGU DoS Entropy Evaluation Script
...
Follows the exact same pipeline structure as evaluate_dos_mirgu.py
"""
import os
...
# ---------------------------------------------------------
# MIRGU DoS ENTROPY CONSTANTS
# (Computed from benign-only training data:
#  dos_dataset/Mirgu_DoS_arbitrated_data/Target/train.csv)
# ---------------------------------------------------------
TRAIN_MEAN = 8.1876
TRAIN_STD  = 0.1162
K          = 3.5
WINDOW     = 0.0376
LOWER      = TRAIN_MEAN - K * TRAIN_STD   # 7.7809
UPPER      = TRAIN_MEAN + K * TRAIN_STD   # 8.5943
...
# (Same logic as evaluate_dos_mirgu.py save_preds)
# ---------------------------------------------------------
def save_preds(pass_num, tracksheet, traffic_rows, output_path, preds):
    """
    traffic_rows: list of raw CSV rows (from build-like reading)
    preds: array of window predictions (0/1) — NOT frame-level here,
           so we map window preds back to packets differently.
    
    For entropy: we write packet-level output + update tracksheet.
    """

    # --- Write detailed prediction output CSV ---
    output_rows = []
    # We don't have 29-packet frames here; instead we have
    # window_indices mapping. This function is called with
    # (df, window_indices, preds) — see run() below.
    # We handle it inside run() and call this for tracksheet update only.

    # --- Update tracksheet (same logic as evaluate_dos_mirgu.py) ---
    ...
    # pred_labels passed in via traffic_rows argument (reused param)
    pred_labels = traffic_rows  # list of "A"/"B" strings
    ...
    if n_pred < n_df:
        print(f"[WARN] pred_labels shorter than packet CSV: {n_pred} vs {n_df}. "
              f"Filling remaining using operation_label.")
        ...
        print(f"[WARN] pred_labels longer than packet CSV: {n_pred} vs {n_df}. "
              f"Truncating extra predictions.")
    ...
    tracksheet_dir = "tracksheets_mirgu"
...
def plot_confusion(cm, pass_num, y_test, preds):
    ...
    plt.savefig("./CF_target/mirgu_dos_entropy_cf_pass_{}.png".format(pass_num))
    plt.close()
...
def run(params):
    ...
    print(f"\nDataset: MIRGU DoS (Entropy) | Mean: {TRAIN_MEAN} | Std: {TRAIN_STD} "
          f"| K: {K} | Window: {WINDOW}")
    ...
    try:
        ...
        # If first column looks like a header string, it has a header already
        # If not, re-read without header
        if df.columns[0] not in ["Timestamp", "timestamp", "Time", "time"]:
            ...
            # Safety: if first row is actually a header string
    ...
    # 4. Prediction (Outlier Detection)
    ...
    # 6. Map window predictions back to packet-level pred_labels
    df["pred_label"] = "B"
    ...
    # 8. Update tracksheet (same logic as evaluate_dos_mirgu.py)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="config_dos_mirgu.yaml")
