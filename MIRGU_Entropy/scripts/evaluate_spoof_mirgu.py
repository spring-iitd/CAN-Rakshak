#!/usr/bin/env python3
"""
MIRGU Spoof Entropy Evaluation Script
...
Follows the exact same pipeline structure as evaluate_spoof_mirgu.py
"""
import os
...
# ---------------------------------------------------------
# MIRGU Spoof ENTROPY CONSTANTS
# (Computed from benign-only training data:
#  spoof_dataset/Mirgu_break_spoof_arbitrated_data/Target/train.csv)
# ---------------------------------------------------------
TRAIN_MEAN = 8.1876
TRAIN_STD  = 0.1162
K          = 3.5
...
# (Same logic as evaluate_spoof_mirgu.py save_preds)
# ---------------------------------------------------------
def save_preds(pass_num, tracksheet, pred_labels_list, output_path, preds):
    """
    pred_labels_list: list of "A"/"B" strings for each packet
    Updates the tracksheet CSV with pred_label column.
    """
...
def plot_confusion(cm, pass_num, y_test, preds):
    ...
    plt.savefig("./CF_target/mirgu_spoof_entropy_pass_{}.png".format(pass_num))
    plt.close()
...
def run(params):
    ...
    print(f"\nDataset: MIRGU Spoof (Entropy) | Mean: {TRAIN_MEAN} | Std: {TRAIN_STD} "
          f"| K: {K} | Window: {WINDOW}")
    ...
    try:
        ...
        # If first column doesn't look like a timestamp header, re-read headerless
    ...
    # 8. Update tracksheet (same logic as evaluate_spoof_mirgu.py)
