...
def pipeline_run(config):
    ...
    if steps.get("attack", False):
        ...
        from scripts.adversarial_attack_dos_final_mirgu import run as run_attack
        ...
        if round_num == 0:
            ...
            attack_cfg["packet_level_data"] = (
                f'{config["update"]["tracksheet_dir"]}/dos_test_track_{round_num-1}.csv'
            )
    ...
    if steps.get("decode", False):
        ...
        from scripts.Traffic_decoder_dos_mirgu import run as run_decode
    ...
    if steps.get("evaluate", False):
        ...
        from scripts.evaluate_dos_mirgu import run as run_eval
    ...
    if steps.get("update", False):
        ...
        from scripts.update_labels_dos_mirgu import run as run_update

        update_cfg = config["update"].copy()

        update_cfg["tracksheet"] = (
            f'{config["update"]["tracksheet_dir"]}/dos_test_track_{round_num}.csv'
        )
