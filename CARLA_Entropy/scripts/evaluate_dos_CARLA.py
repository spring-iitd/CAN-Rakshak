#!/usr/bin/env python3
"""
CARLA DoS Entropy Evaluation Script
...
Follows the exact same pipeline structure as evaluate_dos_CARLA.py
"""
import os
...
# ---------------------------------------------------------
# CARLA DoS ENTROPY CONSTANTS
# (Computed from benign-only training data:
#  CARLA_dataset/DoS/Target/train.csv)
# ---------------------------------------------------------
TRAIN_MEAN = 5.6106
TRAIN_STD  = 0.1734
K          = 3.5
...
# (Same logic as evaluate_dos_CARLA.py save_preds)
# ---------------------------------------------------------
...
def plot_confusion(cm, pass_num, y_test, preds):
    ...
    plt.savefig("./CF_target/CARLA_dos_entropy_cf_pass_{}.png".format(pass_num))
    plt.close()
...
def run(params):
    ...
    print(f"\nDataset: CARLA DoS (Entropy) | Mean: {TRAIN_MEAN} | Std: {TRAIN_STD} "
          f"| K: {K} | Window: {WINDOW}")
